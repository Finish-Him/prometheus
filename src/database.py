...
def load_dreamleague() -> Dict[str, Any]:
    ...
    if client:
        try:
            # Get tournament
            tournament = client.table("tournaments").select("*").eq("id", "dreamleague_s27").single().execute()

            # Get teams
            teams = (
                client.table("tournament_teams").select("*, teams(*)").eq("tournament_id", "dreamleague_s27").execute()
            )

            # Get schedule
            schedule = (
                client.table("schedule")
                .select("*")
...
def load_pro_teams() -> Dict[str, Any]:
    ...
    if client:
        try:
            result = client.table("teams").select("*, team_stats(*)").order("rating", desc=True).execute()
...
def load_pro_players() -> Dict[str, Any]:
    ...
    if client:
        try:
            result = client.table("players").select("*, teams(name, tag)").order("winrate", desc=True).execute()
...
def load_bets(user_id: str = "default_user") -> Dict[str, Any]:
    ...
    if client:
        try:
            # Get bankroll
            bankroll_result = client.table("bankroll").select("*").eq("user_id", user_id).single().execute()

            # Get bets
            bets_result = (
                client.table("bets").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
            )
...
def update_bankroll(user_id: str, amount: float, operation: str = "set") -> bool:
    ...
    try:
        if operation == "set":
            client.table("bankroll").update({"balance": amount, "updated_at": datetime.now().isoformat()}).eq(
                "user_id", user_id
            ).execute()
        elif operation == "add":
            result = client.table("bankroll").select("balance").eq("user_id", user_id).single().execute()

            current = result.data.get("balance", 0) if result.data else 0
            new_balance = current + amount

            client.table("bankroll").update({"balance": new_balance, "updated_at": datetime.now().isoformat()}).eq(
                "user_id", user_id
            ).execute()
...
def settle_bet(bet_id: str, result: str, profit: float) -> bool:
    ...
    try:
        client.table("bets").update(
            {"status": result, "result": result, "profit": profit, "settled_at": datetime.now().isoformat()}
...
def get_team_head_to_head(team_a_id: int, team_b_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        result = (
            client.table("matches").select("*").eq("team_id", team_a_id).eq("opponent_team_id", team_b_id).execute()
        )
