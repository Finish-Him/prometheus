...
class OpenRouterProvider(AIProvider):
    ...
    async def call_model(
        self, model: str, prompt: str, system: str = None, temperature: float = 0.7, max_tokens: int = 4000
    ) -> Dict:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.BASE_URL, headers=self.headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        return {"success": False, "model": model, "error": f"API error {response.status}: {error}"}
...
"""
    ...
    async def analyze_match(self, team_a: str, team_b: str, match_data: Dict = None, format_type: str = "Bo3") -> Dict:
        """
        ...
        for i, result in enumerate(results):
            ...
            if result.get("success"):
                analyses.append(
                    {"model": result["model"], "content": result["content"], "usage": result.get("usage", {})}
                )
    ...
    async def quick_analysis(self, team_a: str, team_b: str, match_data: Dict = None) -> Dict:
        """Quick analysis using cost-effective model (Gemini Flash)."""

        prompt = self._build_quick_prompt(team_a, team_b, match_data)

        result = await self.provider.call_model("flash", prompt, self.SYSTEM_PROMPT, temperature=0.5, max_tokens=1500)

        if result.get("success"):
            return {"success": True, "match": f"{team_a} vs {team_b}", "analysis": result["content"], "model": "flash"}
...
"""
        ...
        result = await self.provider.call_model("gemini", prompt, self.SYSTEM_PROMPT, temperature=0.3)
    ...
    def _build_analysis_prompt(self, team_a: str, team_b: str, match_data: Dict, format_type: str) -> str:
...
"""
    ...
    def _build_quick_prompt(self, team_a: str, team_b: str, match_data: Dict = None) -> str:
...
Responda em português, máximo 200 palavras."""
    ...
    def _build_consensus(self, analyses: List[Dict], team_a: str, team_b: str) -> Dict:
...
async def analyze_live_draft(
    radiant_picks: List[str], dire_picks: List[str], radiant_bans: List[str] = None, dire_bans: List[str] = None
) -> Dict:
    """Analyze live draft composition."""
    analyzer = MultiAIAnalyzer()
    return await analyzer.analyze_draft(radiant_picks, dire_picks, radiant_bans, dire_bans)
...
def sync_analyze_draft(
    ...
    return asyncio.run(analyze_live_draft(radiant_picks, dire_picks, radiant_bans, dire_bans))
