...
class TestHeroMap:
    ...
    def test_hero_primary_attr_is_valid(self):
        ...
        for hero_id, hero_data in HERO_MAP.items():
            assert (
                hero_data["primary_attr"] in valid_attrs
    ...
    def test_hero_attack_type_is_valid(self):
        ...
        for hero_id, hero_data in HERO_MAP.items():
            assert (
                hero_data["attack_type"] in valid_types
            ), f"Hero {hero_id} has invalid attack_type: {hero_data['attack_type']}"

    def test_hero_roles_is_list(self):
        for hero_id, hero_data in HERO_MAP.items():
            assert isinstance(hero_data["roles"], list), f"Hero {hero_id} roles should be a list"

    def test_hero_roles_are_non_empty(self):
        for hero_id, hero_data in HERO_MAP.items():
            assert len(hero_data["roles"]) > 0, f"Hero {hero_id} has empty roles list"

    def test_hero_names_are_strings(self):
        for hero_id, hero_data in HERO_MAP.items():
            assert isinstance(hero_data["name"], str), f"Hero {hero_id} name is not a string"
            assert len(hero_data["name"]) > 0, f"Hero {hero_id} has empty name"
