...
VPS_CONFIG = {"host": "72.62.9.90", "port": 22, "username": "root", "password": "Moises@24512987"}
...
def main():
    ...
    status, output, error = execute_command(
        ssh, "tail -50 /opt/prometheus/logs/prometheus-error.log 2>/dev/null || echo 'No error log file'"
    )
    print(output)
