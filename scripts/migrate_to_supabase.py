...
def migrate_team_stats(client: Client) -> int:
    ...
    for team in teams_list:
        ...
        try:
            client.table("team_stats").upsert(record, on_conflict="team_id,period").execute()
            count += 1
        except Exception as e:
...
def migrate_team_heroes(client: Client) -> int:
    ...
    for team in teams_list:
        ...
        for hero in heroes[:10]:  # Top 10 heroes per team
            ...
            try:
                client.table("team_heroes").upsert(record, on_conflict="team_id,hero_id").execute()
                count += 1
            except Exception as e:
...
def initialize_bankroll(client: Client) -> bool:
    ...
    record = {"user_id": "default_user", "balance": 1000.00, "initial_balance": 1000.00}
