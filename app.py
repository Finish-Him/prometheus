...
# Page Config
st.set_page_config(
    page_title="Prometheus V7 - Dota 2 Analytics", page_icon="🔥", layout="wide", initial_sidebar_state="expanded"
)
...
def render_dreamleague():
    ...
    try:
        ...
        from notifications import (
            format_countdown,
            get_countdown_color,
            get_hours_until_match,
        )
        from odds_tracker import calculate_kelly, get_tracker
    ...
    with tab1:
        ...
        if not matches:
            ...
            for match in matches[:12]:
                ...
                # Card com logos
                logo_html_a = (
                    f"<img src='{logo_a}' style='width: 24px; height: 24px; border-radius: 4px; margin-right: 6px;'>"
                    if logo_a
                    else ""
                )
                logo_html_b = (
                    f"<img src='{logo_b}' style='width: 24px; height: 24px; border-radius: 4px; margin-left: 6px;'>"
                    if logo_b
                    else ""
                )
    ...
    with tab2:
        ...
        for team in teams:
            ...
            logo_html = (
                f"<img src='{team_logo}' style='width: 32px; height: 32px; border-radius: 4px; margin-right: 10px;'>"
                if team_logo
                else ""
            )
                ...
                for i, player_basic in enumerate(roster_basic[:5]):
                    ...
                    for p in current_roster:
                        if (
                            ...
                            or player_name.lower() in p.get("name", "").lower()
                        ):
                            player_stats = p
                            break
            ...
            for odds_file in odds_files:
                try:
                    ...
                    while i < len(lines):
                        ...
                        # Detectar times
                        if "Team Spirit" in line:
                            team_a = "Team Spirit"
                        elif "Team Liquid" in line:
                            team_a = "Team Liquid"
                        elif line == "Team OG" or "OG" == line:
                            team_a = "OG"
                        elif "BetBoom" in line:
                            team_a = "BetBoom Team"

                        if "1win" in line:
                            team_b = "1win"
                        elif "Nemesis" in line:
                            team_b = "Team Nemesis"
                        elif "Tidebound" in line:
                            team_b = "Team Tidebound"
                        elif "Runa" in line:
                            team_b = "Runa Team"
                        ...
                        with st.expander(f"🎲 Mercados - {team_a} vs {team_b}"):
                            ...
                            if total_kills:
                                ...
                                if "over" in total_kills:
                                    with cols[0]:
                                        st.markdown(
                                            f"⬆️ {total_kills['over']['line']}: **{total_kills['over']['odd']}**"
                                        )
                                if "under" in total_kills:
                                    with cols[1]:
                                        st.markdown(
                                            f"⬇️ {total_kills['under']['line']}: **{total_kills['under']['odd']}**"
                                        )
    ...
    with tab4:
        ...
        match_options = ["-- Selecione uma partida --"] + [
            f"{m.get('team_a')} vs {m.get('team_b')} ({m.get('time_brt')} BRT)" for m in matches_list
        ]
    ...
    with tab1:
        ...
        with col2:
            ...
            bookmaker = st.selectbox(
                "Casa", ["bet365", "Betano", "Betfair", "Pinnacle", "1xBet", "Rivalry", "GG.bet", "Stake"]
            )
...
def render_match_hub():
    ...
    try:
        from src.analytics import (
            ...
            get_dreamleague_schedule,
            get_dreamleague_teams_analysis,
            get_team_hero_pool,
        )
        from src.hero_mapper import get_hero_image_url, get_hero_name
        ...
        if dl_live:
            ...
            if all_live:
                ...
                for game in all_live[:3]:
                    st.write(
                        f"• {game['radiant_team']['name']} vs {game['dire_team']['name']} - {game.get('game_time_formatted', '0:00')}"
                    )
            else:
...
def render_team_rankings():
    ...
    for i, team in enumerate(teams):
        ...
        with st.expander(
            f"{i+1}. {tier_emoji} **{team.get('name')}** - ⭐ {team.get('rating', 0):.0f} | {form.get('form_tier', '?')}"
        ):
            col1, col2, col3 = st.columns(3)
...
def render_analytics_2025():
        ...
        if USE_DATABASE and supabase_data:
            try:
                ...
                if client:
                    ...
                    result = client.table("picks_bans_2025").select("hero_id, is_pick").limit(100000).execute()
                    ...
                    if result.data:
                        ...
                        if "hero_id" in df.columns and len(df) > 0:
                            ...
                            hero_stats = pd.DataFrame({"Picks": picks, "Bans": bans}).fillna(0).astype(int)
    ...
    with tab4:
        ...
        with col1:
            ...
            months_data = master_data.get("months", {})
            matches_by_month = {m: d.get("summary", {}).get("total_matches", 0) for m, d in months_data.items()}

            import pandas as pd

            df = pd.DataFrame([{"Mês": k, "Partidas": v} for k, v in matches_by_month.items()])
            if not df.empty:
                st.line_chart(df.set_index("Mês"))

        with col2:
            st.markdown("**⚔️ Teamfights por Mês**")
            tf_by_month = {m: d.get("summary", {}).get("total_teamfights", 0) for m, d in months_data.items()}

            df_tf = pd.DataFrame([{"Mês": k, "Teamfights": v} for k, v in tf_by_month.items()])
            if not df_tf.empty:
